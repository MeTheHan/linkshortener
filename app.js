...
function setupEventListeners() {
    ...
    // URL değiştiğinde result'ı gizle
...
function toggleCustomCodeSection(show = null) {
    const section = document.getElementById('customCodeSection');
    const icon = document.getElementById('toggleIcon');
    
    if (show === null) {
        isCustomCodeVisible = !isCustomCodeVisible;
    } else {
        isCustomCodeVisible = show;
    }
    
    if (isCustomCodeVisible) {
    ...
    console.log('📂 Özel kod bölümü:', isCustomCodeVisible ? 'Açık' : 'Kapalı');
}
...
async function shortenUrl() {
    ...
    try {
        ...
        if (customCode) {
            ...
            // Rastgele benzersiz kod oluştur
...
async function generateUniqueShortCode() {
    ...
    // 10 denemede benzersiz bulamazsa uzun kod üret
